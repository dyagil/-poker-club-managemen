...
def update_app_file():
    ...
    # מוצאים את כל הקוד של אייג'נטים 
    agents_pattern = r"""@app.route\('/agents', methods=\['GET'\]\)
@login_required
def agents\(\):.*?return render_template\('agents\.html', agents=agents_list, active_page='agents'\)"""
    
    # הקוד החדש המתוקן לגמרי
...
def agents():
    ...
    print("בוצע שכתוב מלא לקוד האייג'נטים")
